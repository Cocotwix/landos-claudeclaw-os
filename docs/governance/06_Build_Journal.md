...
## 2026-06-28 — Verification-state contradiction (2123 Panola) — root cause: APN parse corruption

**Symptom:** Deal Card showed Realie-verified facts (APN 16 038 07 001, DeKalb, 5.02ac, R100, Realie source) while header/DD/Strategy/Market/Comps/Pre-Call said unverified_lead / "Local Area Context, Not Parcel Verified" / strategy blocked / comps not_run.
**Root cause (proven):** `extractPropertyArgs` corrupted the re-resolve input. `buildIdentityText` lists the APN line before the address line; a space-containing APN ("APN: 16 038 07 001") followed by an address line starting with a house number ("2123 Panola Road") was MERGED into a corrupt APN ("16 038 07 001 2123") and the address was dropped — because the APN regex used `\s+` (which matches newlines) between numeric segments. The corrupt APN failed Realie lookup → report `parcelVerified:false` while the card kept its genuine verified facts → contradictory sections. (Provider output was correct: clean APN+county+state and the address both verify; the bug was our parser.)
**Canonical state:** the persisted report's verification is canonical; all departments already read it via the report GET synthesis. The contradiction was a FALSE-NEGATIVE re-verify, not divergent readers — fixing the false-negative makes every section agree.
**Fix (parser, line ~171 duke-preflight.ts):** APN multi-segment separator changed from `\s+` to `[^\S\n]+` (horizontal whitespace only) so a space-APN cannot span lines / absorb the next line's house number. One-line structural fix; no provider/downstream logic changed.
**Regression tests:** (1) `extractPropertyArgs` on the exact APN-led multi-line text → clean apn "16 038 07 001" (not merged) + county/state; single-line space-APN still parses. (2) deal-card-report rerun with a space-APN + address + a resolver that verifies only the CLEAN apn → `parcelVerified:true`, status not "Local Area Context", persists/reloads consistent.
**Live (2123 Panola, deal 65, after fix):** parcelVerified=true (Realie.ai non-credit); comps realie:collected (2 sold) + zillow:collected (190 active); providers realie+zillow connected; DD readiness 47% (advanced, not blocked). No contradictory labels remain. Suite 1434 green; tsc + build clean.

## 2026-06-29 — Verified identity now propagates to the card (identity != DD completeness)

**Follow-on to the APN parse fix:** even once the report verified 2123 Panola, the subject CARD stayed `verification_status=unverified_lead` / `kanban=needs_parcel_verification` (the report run never upgraded the card), so the header read "unverified_lead" while the report + DD facts read verified — a residual contradiction.
**Root rule enforced:** parcel-identity verification is SEPARATE from DD completeness. A Realie-verified parcel must read "identity verified" on the card even when DD is incomplete.
**Fixes:**
- `deal-card-report.ts`: on a successful verification with strong identity (APN or property id), the report now upgrades the subject property card via `upsertCardFromDukeRun({ cardId, verified:true, … })` → `verified_property` (cardId-targeted, never creates a duplicate; only runs on parcelVerified so it never downgrades).
- `property-card.ts`: kanban now advances off `needs_parcel_verification → needs_seller_discovery` when a card becomes verified (previously only `new_lead → researching`), so a verified parcel stops reading as "needs verification."
**Language audit (deal 65, verified):** no contradictory identity-unverified labels remain — `unverified_lead`, `needs_parcel_verification`, "Local Area Context, Not Parcel Verified", "parcel identity not verified", "market target not defined" all clean. Remaining DD-gap / "core land facts incomplete" language is DD-completeness (the separate, legitimate axis). Market summary correctly targets the verified DEKALB County and notes market-pulse area context is "separate from parcel verification."
**Live (2123 Panola, deal 65):** BEFORE card unverified_lead / needs_parcel_verification → AFTER verified_property / needs_seller_discovery; report parcelVerified=true; comps realie:collected + zillow:collected. Header, DD, Market, Strategy, Pre-Call, Comps all agree. Regression test added (verified report upgrades card + advances kanban). Suite 1435 green; tsc + build clean.

## 2026-06-29 — DD recovery: governance philosophy + Executive Summary synthesis (shell → analyst)

**Phase 1–2 (governance, done first):** rewrote `07_Product_Principles.md` and added to `01_Vision.md` — LandOS now explicitly retrieves, INTERPRETS, SYNTHESIZES, estimates from verified evidence, ranks strategies, and gives operator-ready pre-call intelligence with explicit confidence labels. "Unknown is better than guessed" / "facts over guesses" replaced with "a clearly-labeled estimate beats conservative silence." Codified the verification model: a parcel is VERIFIED for pre-call use when a trusted provider resolves the input into a real parcel record (Realie address / APN+county+state / parcel-specific comps); verification UNLOCKS the pipeline; title/legal/utilities/zoning/access/buildability remain labeled DD items but do NOT block synthesis. Identity verification is separate from DD completeness.
**Phases 5/6/8 (synthesis, the core shell→analyst lever):** new `deal-card-executive-summary.ts` — pure synthesizer over the persisted report producing an operator-ready brief: headline, what-it-is, why-interesting, **Market Pulse** (Realie sold / Zillow active counts + p25/median/p75 PPA band + supply/demand/liquidity + confidence + interpretation, no placeholders when data exists), **Preliminary Acquisition Range** (40–60% of estimated market value = verified median PPA × verified acreage; conservative/mid/range, 40% + 60% points, confidence, assumptions, value-up/value-down variables — only when verified sold comps exist; honestly withheld otherwise), strongest strategy + why, top risks (incl. verified FEMA/NWI/slope), seller questions, verify-before-offer, next steps. Wired into the report GET response (`executiveSummary`) and rendered as the FIRST section of the Deal Card UI.
**Live acceptance (both addresses):** 2123 Panola → verified, "5.02 ac DeKalb, target $432,973–$649,459" (2 sold, conf LOW correctly flagged for sparse/suburban-skewed comps); 514 Wilderness Trl, Liberty SC → verified, "2.52 ac Anderson, target $70,607–$105,911" (8 sold, conf HIGH). Both: Realie facts + comp band + Zillow active + range + strategy + risks + seller Qs. Tests +5; suite 1440 green; tsc + web build clean.
**Honest scope:** Phases 3 (field-by-field pipeline audit), 4 (Google imagery regression), 7 (deeper per-strategy ranking), 9 (full cross-panel consistency sweep), 10 (iterative product review) remain as continuation on this foundation.

## 2026-06-29 — DD operational: Strategy ranking + Deal Economics in the Executive Summary

Extended the Executive Summary synthesizer (`deal-card-executive-summary.ts`) so the live Deal Card reads like an analyst file:
- **Strategy ranking**: 8 property-specific lanes (quick flip, retail resale, improved-property resale, land-home package, subdivision, neighbor sale, hold, pass), each scored deterministically from verified acreage / land use / improvement presence / comp band / liquidity / FEMA-NWI-slope / access, with viability + reason + risk + confidence + what-must-be-verified, ranked; the top lane drives "strongest strategy."
- **Deal Economics (preliminary)**: estimated value low/mid/high (verified PPA band × acreage), acquisition band (40–60%), GROSS pre-cost spread, confidence, assumptions, the missing cost items, and why final underwriting comes post-discovery — only when verified comps exist, honestly unavailable otherwise.
- Both wired into the report API + rendered in the Deal Card Executive Summary UI section.
**Live (both addresses):** 2123 Panola → top strategy Retail resale (high), value 372k/1.08M/1.79M, spread 433k–649k, conf LOW (sparse suburban comps, flagged); 514 Wilderness Trl, Liberty SC → top strategy Quick flip (high), value 94.5k/176.5k/927k, acquire 70.6k–105.9k, conf HIGH. Both produce full Exec Summary + range + strategy + economics + risks + 5 seller Qs + 8 verify-before-offer + next steps.
**Imagery finding (honest):** Google IS configured; static/Street-View capture runs when Realie returns coordinates (deal 65 captured) but not for a verified parcel without coords (deal 70) — address-geocoding for SUPPORTING imagery (allowed; imagery ≠ identity) is a clean future enhancement; Maps/Street-View/Earth links remain available from the address.
**Honest remaining scope:** full field-by-field source-to-UI audit table, the imagery geocoding enhancement, deeper per-strategy confidence, and a full cross-panel consistency sweep remain. Tests +8 (suite 1443); tsc + web build clean.

## 2026-06-29 — Google imagery regression ROOT-CAUSED + fixed (same .env key bug)

**Symptom:** imagery "disappeared" / inconsistent (deal 65 captured 2, deal 70 captured 0).
**Root cause:** `capturePropertyVisuals` reads its Google key from `deps.env ?? process.env`, and the report called it WITHOUT `env` → on the live server (key in the `.env` FILE, not `process.env`) `googleVisualConfigured(env)` returned false → "not configured" → NO capture. The SAME process.env-vs-.env-file bug that hit Realie/Zillow. Plus: a verified parcel with no provider coordinates (514 Wilderness) had nothing to capture.
**Fixes:** (1) the report now passes the `.env`-resolved Google key (`resolveGoogleVisualEnv()`) to the capture; (2) `county-geocode` now also returns lat/lng, and the report geocodes the VERIFIED address for IMAGERY ONLY (supporting context, never identity) when the parcel provider returns no coordinates, falling back to the card's original input address when the situs isn't normalized.
**Live:** deal 65 (2123 Panola) 2/4 captured; deal 70 (514 Wilderness, no Realie coords) now **2/4 captured** (satellite + Street View; the other 2 are keyless Maps/Earth links). Imagery reliably appears for verified parcels. Tests +2 (geocode lat/lng); suite 1445 green; tsc + web build clean.

## 2026-06-29 — Browser Intelligence → Market Pulse; DD Department COMPLETE (pre-call)

- **Browser Intelligence integration:** `summarizeGrowthDrivers()` classifies the already-collected public-web evidence into land-value growth drivers (residential/subdivisions, commercial, industrial/logistics, infrastructure/highway, utilities, rezoning/annexation, schools/hospitals, major land deals), produces an OPERATOR summary (not a headline dump), and ends with "what this means for Tyler." Wired into the Executive Summary's Market Pulse + the report API + Deal Card UI.
- **Capability matrix:** `docs/governance/08_DD_Capability_Matrix.md` — permanent source-to-UI coverage matrix (R/M/P/S/D/T/A) for every DD capability, with honest provider limitations separated from implementation gaps.
- **Live acceptance (both):** 2123 Panola → verified, imagery 2/4, Retail-resale #1, range $433k–$649k, growth "strengthening (industrial/commercial/schools)"; 514 Wilderness, Liberty SC → verified, imagery 2/4, Quick-flip #1, range $70.6k–$105.9k, growth "residential subdivisions/rezoning." Both produce Executive Summary + facts + imagery + FEMA/NWI/USGS + Market Pulse + growth summary + comps + acquisition range + Deal Economics + ranked strategies + risks + seller questions + next actions + sources — no shell sections.
- Tests +2 (suite 1447); tsc + web build clean.
- **DD Department marked operationally complete for pre-call use; frozen** (future: bugs, provider changes, small UX, deferred Interactive Map).

## 2026-06-29 — Comp validation: vacant-land comps not inflated by house sales

`realie-comps.ts` now VALIDATES sold comps for the subject's type: a vacant subject excludes improved (house) sales — those with `yearBuilt` — so residential sales can't inflate the land price-per-acre. Result now carries `excluded[]` (with reasons) + `validationNote` for transparency. `deal-card-report` derives `subjectImproved` from verified building area / year built / land use and passes it. Tests +2 (vacant excludes houses; improved retains). Suite 1449 green; tsc + build clean.
**Honest limitation (live):** the filter only excludes a comp when the comp ROW carries a building signal (`yearBuilt`). Realie's comparables endpoint does not reliably return building data per comp, so for some markets (e.g. DeKalb / 2123 Panola, median ~$215k/ac) improved comps can't be detected and the band may still be high. Logic is correct + ready; effectiveness is gated by provider comp-row completeness. A price-per-acre outlier guard for vacant subjects is the honest next step.

## 2026-06-29 — Vacant-land comp safety: IQR high-outlier guard (2nd pass)

Added a vacant-subject 2nd-pass guard to `validateSold`: with >=4 comps, trims extreme high price-per-acre outliers (> p75 + 1.5·IQR) — likely improved/anomalous sales — even when comp rows lack `yearBuilt`. +test (trims a $180k/ac no-yearBuilt outlier from a vacant set). Suite 1450 green; tsc + build clean.
**Honest live result:** does NOT change 2123 Panola (only 2 comps — below the threshold; can't outlier-detect 2 points) or 514 Wilderness (8 comps but genuinely SPREAD, so $368k isn't a statistical outlier). 2123 Panola's inflated band remains a real, unresolved issue: it needs comp BUILDING data (Realie omits it) or a WIDER comp radius for more data points — not solvable by an outlier guard alone. Parked + documented.
