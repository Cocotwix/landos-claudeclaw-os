...
/**
 * LandOS patch 09 — keyless manual search. Queries the host's free geocoder
 * (LandOS canonical subjects, then Nominatim), flies to the best candidate,
 * and reports its label. A LandOS subject candidate carries the property's
 * canonical coordinates — never a manufactured street-address geocode.
 */
async function keylessSearchAndFlyTo(viewer, query, options = {}) {
  ...
  const response = await fetch(`/api/gev/geocode?q=${encodeURIComponent(query)}`);
  if (!response.ok) throw new Error(`Keyless geocoding failed: HTTP ${response.status}`);
  const data = await response.json();
  const candidate = data?.candidates?.[0] || null;
  if (!candidate) return null; // honest not-found

  if (!mayFly()) return CANCELLED_SEARCH;
  const duration = finitePositive(options.duration) || 3.0;
  // Precise addresses and LandOS subjects frame close; larger places frame
  // from their bounding-box span (capped so a state never flies to space).
  let range = finitePositive(options.range) || 2500;
  if (!finitePositive(options.range) && Array.isArray(candidate.boundingBox) && candidate.boundingBox.length === 4) {
    const [south, north, west, east] = candidate.boundingBox.map(Number);
    const spanKm = Math.max(
      Math.abs(north - south) * 111,
      Math.abs(east - west) * 111 * Math.cos((candidate.lat * Math.PI) / 180),
    );
    if (Number.isFinite(spanKm) && spanKm > 0) {
      range = Math.min(60000, Math.max(1200, spanKm * 1400));
    }
  }
  flyToLandmark(viewer, candidate.lat, candidate.lon, {
    range,
    buildingHeight: 0,
    duration,
    onStart: options.onStart,
  ...
  return {
    ...
    navigationMode: candidate.source === 'landos' ? 'landos-subject' : 'keyless-geocode',
    rangeM: range,
  };
}
...
export async function searchAndFlyTo(viewer, query, options = {}) {
  ...
  // LandOS patch 09: manual search must work WITHOUT Google. Keyless mode
  // routes through the host's governed free geocoder (/api/gev/geocode —
  // LandOS canonical subject first, then Nominatim). An empty result stays an
  // honest null ("Location not found"), never the default demo location.
