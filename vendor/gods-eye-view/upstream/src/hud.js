...
export class IntelHUD {
  ...
  constructor(viewer) {
    this.viewer = viewer;
    // LandOS does not mount the OpenAI HUD-summary endpoint. Hosted mode keeps
    // the deterministic local summary and never starts the network cadence, so
    // a disabled provider cannot create recurring 404s or model-call attempts.
    this._aiSummaryEnabled = window.__GEV_HOSTED__ !== true;
    this._visible = false;
    ...
    this._onCameraMoveEnd = () => {
      ...
      if (this._aiSummaryEnabled && !this._firstSummaryKicked && this._visible && this._latestMetrics) {
