...
export class MapStackController {
  constructor(viewer, {
    ...
    onChange = null,
    onError = null,
    // LandOS patch 08: provider hooks for on-demand (post-boot) activation.
    // - createGoogleTileset: async () => Cesium3DTileset|null. Called the
    //   first time the photoreal stack is ACTIVATED with no tileset loaded.
    //   This is what makes the billable Google session lazy: opening the app
    //   on OSM/Bing creates no session; selecting Google 3D does. Returning
    //   null (e.g. local session safeguard refused) fails the switch honestly.
    // - googleConfigured: whether a Google browser key exists at all. With a
    //   hook present but no key, the chip renders as "Setup required".
    providerHooks = null,
  } = {}) {
    this.viewer = viewer;
    this.googleTileset = googleTileset;
    this.cesiumToken = String(cesiumToken || '').trim();
    this._hooks = providerHooks && typeof providerHooks === 'object' ? providerHooks : null;
  ...
  _unavailableReason(stack) {
    // LandOS patch 08: distinguish "missing configuration" from "broken".
    if (stack?.requiresIon) {
      return 'Setup required — add a Cesium ion token (assets:read) in Settings → God\'s Eye View';
    }
    if (stack?.kind === 'photoreal' && this._hooks && !this._hooks.googleConfigured) {
      return 'Setup required — add a Google Map Tiles key in Settings → God\'s Eye View';
    }
  ...
  isStackAvailable(id) {
    ...
    if (stack.kind === 'photoreal') {
      // LandOS patch 08: with a creation hook and a configured key, the stack
      // is selectable even though the (billable) tileset is created lazily on
      // first activation.
      return !!this.googleTileset
        || Boolean(this._hooks?.createGoogleTileset && this._hooks?.googleConfigured);
    }
  ...
  /**
   * LandOS patch 08: accept a Cesium ion token after construction (operator
   * configured it in Settings while the app was running). Makes the Bing
   * stacks selectable immediately — no rebuild or restart.
   * @param {string} token
   */
  setCesiumToken(token) {
    const next = String(token || '').trim();
    if (next === this.cesiumToken) return;
    this.cesiumToken = next;
    if (next) Cesium.Ion.defaultAccessToken = next;
    this._emitChange(this._isSwitching ? 'switching' : 'ready');
  }
  ...
  async _activatePhotoreal(gen) {
    // LandOS patch 08: lazy, gated creation of the Google tileset — the
    // billable root-tileset session happens on ACTIVATION, never merely on
    // app open with a different stack selected.
    if (!this.googleTileset && this._hooks?.createGoogleTileset) {
      const created = await this._hooks.createGoogleTileset();
      if (gen != null && gen !== this._switchGen) return;
      if (!created) {
        throw new Error(this._hooks.googleConfigured
          ? 'Google 3D Tiles unavailable (session safeguard or load failure)'
          : this._unavailableReason(this.getStack('photoreal')));
      }
      this.viewer.scene.primitives.add(created);
      this.googleTileset = created;
    }
