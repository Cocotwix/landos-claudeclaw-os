...
export function mapStackChipModel(stack, activeId) {
  ...
  // LandOS patch 08: an unconfigured provider is "Setup required", never
  // silently missing or apparently broken. Every unavailable chip gets the
  // SETUP badge; the tooltip carries the exact reason + where to configure.
  const setupRequired = !available && /setup required/i.test(unavailableHint);
  return {
    id: String(stack?.id ?? ''),
    label,
    available,
    ...
    requiresIon,
    requirement: !available ? (setupRequired ? 'SETUP' : (requiresIon ? 'ION' : '')) : '',
    setupRequired,
    unavailableHint,
...
export function renderMapStackChips(container, stacks, { activeId = null, onSelect = null, doc } = {}) {
  ...
  for (const model of models) {
    ...
    chip.addEventListener('click', () => {
      if (!model.available) {
        // LandOS patch 08: clicking a setup-required source raises a host
        // event so the shell can offer a direct "Open Settings" action.
        try {
          globalThis.dispatchEvent?.(new CustomEvent('gev:setup-required', {
            detail: { kind: 'map-stack', id: model.id, label: model.label, reason: model.unavailableHint },
          }));
        } catch { /* host event is best-effort */ }
        return;
      }
      onSelect?.(model.id);
    });
