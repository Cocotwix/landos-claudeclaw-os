...
| Default governance is autonomy. The only approval gates are new secrets, `.env` or credential changes, API keys/passwords, paid APIs, external accounts, money, destructive deletes, `git push`, and deployments. | LandOS employees should continue until the business outcome is complete; micro-prompts and approval-drip block business progress without adding safety. |
