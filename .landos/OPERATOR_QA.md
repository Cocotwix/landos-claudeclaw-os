...
### 2026-08-25 - God's Eye View: new top-level sidebar department (vendored upstream, security-gated)

- Scope: full integration of the open-source God's Eye View app (bilawalsidhu/gods-eye-view @ 880a672, upstream HEAD) as the `/dept/gods-eye-view` department. Security gate ran first and PASSED (see `docs/landos/GODS_EYE_VIEW_SECURITY_AUDIT.md`); no npm install (deny-listed) — hash-verified vendored artifacts instead (`vendor/gods-eye-view/VENDORED-ARTIFACTS.md`).
- Browser visual acceptance (live localhost:3141, operator Chrome):
  - visible_assertion= "God's Eye View" appears in the Departments sidebar and opens the complete upstream console inside the SPA (no iframe): sensor-scope map over Austin with live OSM tiles, tactical HUD (MGRS/GSD/ALT readouts ticking), DATA LAYERS panel listing Live Flights/Military Flights/Earthquakes/Satellites/Space Missions/Street Traffic/…; enabling Earthquakes drew live USGS magnitude chips (M2.5–M4.4 along the Pacific coast) and enabling Satellites drew live CelesTrak entities with orbital tracks (ISS, COSMOS 1743, TITAN 4B R/B…) served through the bridged `/api/celestrak` proxy (200, x-tle-cache MISS→live).
  - refresh= hard refresh kept the route, the full share-state hash (camera/style/layer state), and the enabled-layer set (earthquakes+satellites re-enabled from the suspend snapshot). Console clean except the documented HUD AI-summary 404 warning (endpoint deliberately not mounted; HUD shows its honest local fallback).
  - Honest states verified: Google 3D Tiles "setup required" banner (keyless map, "No Google request is made until a key is configured"); FIRMS `/api/firms/status` hasKey:false; voice completely absent (no control element, `voiceCommands === null`); Settings → God's Eye View shows key field (Not configured), monthly limit 900, Sessions this month 0/900 with local-safeguard disclaimer.
  - Lifecycle: leaving the route suspends (render loop stopped, all layer polling disabled, layer set saved, DOM detached, `gev-active` styling lifted, credits hidden); returning resumes the same instance (1 `#gev-root`, camera state intact, layers restored) — no duplicate map instances and no new Google session on a route bounce; full destroy after 5 min away.
  - Acquisitions regression: pipeline board, leads, and tabs render unchanged with GEV suspended in the background; runtime healthy throughout.
- Screenshots: `store/operator-qa-canonical/gods-eye-view/` (department open, first-run dialog, live layers on globe, Acquisitions intact, Settings section, post-refresh view).
- Engineering QA: typecheck clean; vite+tsc build clean; dashboard contract suite 147/147; vision-architecture + routing-map suites pass; full vitest 7695 pass / 24 pre-existing fails in in-flight market-intelligence + checkpoint-drift files (untouched, none GEV-related). Not committed.
- Note for QA automation: the app honors upstream's hidden-tab render suspension — a backgrounded Chrome tab legitimately stops the Cesium render loop, so headless/background QA must force `viewer.render()` passes or foreground the tab; this is power-saving behavior, not a defect.

### 2026-08-25 - God's Eye View repository-browser re-acceptance (PASS)

- Root cause repaired: the previous Codex run queried only its empty session browser broker and never invoked LandOS's owned Puppeteer/CDP path. The canonical agent-independent command is now `npm run landos:browser:qa -- --scenario gods-eye-view`; missing session browser tooling alone is not a blocker.
- Browser chain verified live: authenticated health passed; owned Chrome PID 167532 was discovered on CDP 9224; a real QA page was controlled; only that page and CDP client were closed; the managed process and its one pre-existing page remained. Windows process ownership discovery now uses unformatted command-line output plus three bounded retries for transient unreadability while continuing to fail closed for foreign profiles.
- Tier 3 result: PASS, 48/48 live assertions/captures, 0 relevant browser errors. The keyless globe rendered; Google 3D and Bing controls said setup required; Earthquakes and Satellites rendered live USGS/CelesTrak states; FIRMS/AIS showed honest missing-key states; Traffic showed its OpenStreetMap simulation fallback; Settings showed configuration state, 900-session safeguard, and voice disabled. First-run copy also states that voice is disabled rather than advertising an absent microphone control.
- Lifecycle/security: leaving the department set the render loop false and provider calls stayed 5 before/after 3.5 seconds away; Acquisitions remained usable; returning resumed the same object with one `#gev-root` and render loop true. Hard refresh preserved the route and enabled layers. `voiceCommands` was null, getUserMedia calls were 0, voice UI count was 0, and TeleGeography was neither registered nor rendered.
- Fresh governed evidence: `.runtime/landos/qa/browser-gods-eye-view-2026-08-25T20-24-41-879Z/` (`report.json`, `report.md`, eight visually inspected screenshots). Expected missing-provider 503s and route-change cancellations remain recorded as warnings rather than being hidden.
- Hosted HUD summary requests are disabled in LandOS mode; deterministic local text remains and the prior recurring unmounted AI-summary 404 no longer occurs.
- Engineering QA: typecheck clean; Vite plus TypeScript build clean; browser discovery/CDP/managed-launch/cleanup/reporting suites 56/56. Managed restart and health passed. Not committed.

### 2026-08-18 - Slice 11 current-zoning/by-right fix: operator QA on the Fairview acceptance card (deal 89)

- Scope: read-only operator QA of the current Fairview/Williamson County, TN acceptance property's Deal Card (V2 workspace), after the current-zoning-resolution-order fix and the historical-lots-never-decide-the-path fix. No code changed in this pass.
- Result: **PASS as an operator screening tool, with one real gap**. The card gives Tyler a usable, honest acquisition-screening answer for identity, assessor/tax, LandPortal evidence, market context, current zoning, current subdivision rules, subject-specific by-right potential, and Property Development History — all correctly separated from historical proposals and all persisted/visible on a normal page load, no button click required.
- Verified persisted and correct on load (no click needed): property identity (address/owner/APN/acreage/county/ZIP), assessor/tax (assessed value, market value, annual tax, improvements, honest "tax payment status not established" with a named next action), LandPortal evidence (17 visuals, parcel facts, source provenance), market intelligence (acreage-band $/acre, DOM, sell-through), current zoning ("Unresolved", historical R-20 POD / RS-40 / RS-15 statements clearly labeled as never establishing current zoning), current subdivision rules (full rule matrix with section citations and PDF links), subject-specific by-right result ("Likely path: Not established" / "the number of lots this tract would be divided into is not established, so the path cannot be indicated" — correctly NOT driven by the historical 119-lot Kingwood proposal), Property Development History (Kingwood proposal shown with explicit "does NOT establish the current zoning district" framing, open questions listed), diligence queue and next operator actions, official source links (PDFs and government pages) throughout.
- **Real gap found (display layer, not a research failure):** the Slice 11 touch-up's manufactured-home screening, "what current zoning allows," "material zoning restrictions," direct-frontage lot potential, and private-road/private-drive handling are computed and persisted correctly (confirmed via the stored capability result), but the V2 Deal Card panel that renders them (`ZoningSubdivisionCapabilityRun`) holds them in local component state with no fetch-on-mount, so none of that content is visible on a normal page open — only immediately after an operator clicks "Run Zoning & Subdivision," and it disappears again on the next visit/reload. The pre-existing, persisted "Subdivision rules & feasibility" panel does correctly show the fixed by-right path/current-zoning text without a click; it just doesn't carry the newer manufactured-home/allowances/restrictions/frontage content.
- Non-blocking: the "Valuation & comps" toggle and the Overview page's "Open Comps & Valuation →" link do not lead to a distinct comps/valuation view — they render the same "Property & diligence" content. The valuation ANSWER itself (0 accepted comps, land value pending, insufficient closed-sale evidence) is still shown clearly on Overview and in this shared view, so this is a labeling/navigation gap, not a missing answer.
- No console errors on load or on the persisted panels. LandOS runtime healthy throughout.
- Next exact task (not implemented this pass, per read-only QA scope): fetch the latest persisted zoning-subdivision capability result on mount (GET `/api/landos/deal-cards/:id/zoning-subdivision/capability` already exists and returns it) so manufactured-home/allowances/restrictions/frontage/private-road content survives a reload; separately decide whether "Valuation & comps" should route to a distinct view or be relabeled/removed.
