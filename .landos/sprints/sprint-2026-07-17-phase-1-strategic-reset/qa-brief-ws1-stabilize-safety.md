# Independent Browser-QA Brief — ws1-stabilize-safety: Data stabilization, QA isolation, and hard safety boundaries
...
Operator outcome under test: Operating data is protected and recoverable, synthetic QA is physically separate, and every prohibited LandPortal API/MCP, paid, external-outbound, offer-send, and contract-send path is disabled before workflow use.

## Requirements to disprove
- P1-02: Real operating data and QA data/artifacts are visibly and physically separated.
- P1-16: A private encrypted backup/restore path restores business data into a clean local target with row-count and integrity reconciliation.
- P1-S1: LandPortal API, token, manual endpoint, and MCP routes are unavailable at runtime; authenticated read-only browser work is the only approved path.
- P1-S2: Jarvis outbound is owner-only and paid actions, spending, offer sending, and contract sending are prohibited by focused guards.
- P1-S3: Phase 1 surfaces use functional role labels and preserve legacy IDs only as hidden compatibility aliases.

## Required operator journey
1. Confirm operating/QA identity is visible
2. Confirm LandPortal is described as authenticated browser-only
3. Confirm only functional role names appear in Phase 1 surfaces

## Prohibited outcomes
- Must NOT occur: real records used as QA fixtures
- Must NOT occur: LandPortal API/token/MCP runtime reachable
- Must NOT occur: paid action selectable
- Must NOT occur: Jarvis can contact external parties
- Must NOT occur: offer or contract can be sent
- Must NOT occur: important data is overwritten or lost
