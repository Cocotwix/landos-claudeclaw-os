# Independent Browser-QA Brief — ws2-alternate-pointers: APN-only and exact document-link autonomy
...
Operator outcome under test: A parcel identifier with jurisdiction or an exact supported property link can establish the property without a redundant street address or identifier request.

## Requirements to disprove
- ws2-r1: An APN plus sufficient jurisdiction context can establish the subject without a fabricated or operator-supplied street address when uniquely resolvable.
- ws2-r2: An exact supported LandPortal parcel URL is recognized as the property pointer and does not trigger redundant identifier clarification.
- ws2-r3: Alternate-pointer evidence retains provenance and exact-subject association through authoritative admission.
- ws2-r4: Canonical subject state remains stable and every downstream parcel consumer uses it rather than reopening identity.
- ws2-r5: The alternate-pointer Deal Cards settle autonomously as far as available evidence allows and persist through refresh and managed restart.

## Required operator journey
1. Submit APN plus jurisdiction through New Lead without a street address
2. Observe autonomous property search and honest resolution or genuine ambiguity
3. Submit an exact supported LandPortal parcel URL through New Lead
4. Confirm the link establishes the same subject and retains provenance
5. Inspect parcel research, readiness, intelligence, and refresh persistence

## Prohibited outcomes
- Must NOT occur: APN-only intake requires the operator to provide an address when the parcel can be uniquely resolved
- Must NOT occur: Exact LandPortal link intake requests redundant APN or address corroboration
- Must NOT occur: A downstream consumer reopens identity after canonical establishment
- Must NOT occur: Link-derived evidence loses provenance or admits a different parcel
