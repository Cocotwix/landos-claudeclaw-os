...
This is how the owner turns a raw build request into a structured Forge engagement
...
npx tsx src/forge/engagement-cli.ts --title "Date utils" --host "your operating system" "Add a date helper with a test"
...
{ "request": "Add a date helper to src/utils with a test", "title": "Date utils", "host": "your operating system" }
```
...
and the owner decisions in `decisionsNeeded`. Errors: missing/empty `request`
...
Every request runs through `classifyLane()`, which scans for owner-owned
...
- **STOP** — an owner-owned decision is involved. Forge does not build it until
  the owner decides.

The gate is a **conservative triage aid, not a security boundary**. STOP is
intentionally over-eager (better to ask the owner than to act unasked). SAFE means
"no trigger matched" — the operator still applies judgment.

### Red-lane stop list (owner-owned)
...
1. **Engagement Request** — the owner's raw ask plus optional title and host.
...
2. **Assumption Summary** — what Forge heard, the objective, in/out of scope,
   ...
   the success check, and the exact owner decisions needed before building.
...
3. **Milestone Build Plan** — the objective, the lane verdict, the ordered
   build steps (a STOP request gets a leading "STOP: resolve the owner-owned
...
Assumption Summary → Milestone Build Plan → Review Packet → Owner Direction
...
- **Ask at real checkpoints only.** Ask the owner at genuine business-direction
...
2. Confirm the Assumption Summary with the owner for anything non-trivial.
...
owner-owned categories (secrets, financial platform, git push), lists them as
