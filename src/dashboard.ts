...
export function buildDashboardApp(botApi?: Api<RawApi>): Hono {
  ...
  app.post('/api/chat/send', async (c) => {
    ...
    void processMessageFromDashboard(botApi, message, agentId);
    return c.json({ ok: true });
  });
  ...
  // Serve a local report file (PDF/MD) generated by Duke or other agents.
  // Only files within the approved Obsidian vault directory are served.
  // Auth is enforced by the global token middleware above.
  app.get('/api/files/report', (c) => {
    ...
    // Security: only serve files within the approved vault output directory.
    const ALLOWED_PREFIXES = [
      path.resolve('C:\\Users\\tbutt\\Documents\\Obsidian Land OS -Land Acquisitions'),
      path.resolve('C:\\Users\\tbutt\\OneDrive\\Documents\\Obsidian Land OS -Land Acquisitions'),
    ];
    const isAllowed = ALLOWED_PREFIXES.some(
      prefix => resolvedPath === prefix || resolvedPath.startsWith(prefix + path.sep)
    );
    if (!isAllowed) {
      logger.warn({ resolvedPath }, 'Report file request outside allowed directory');
      return c.json({ error: 'Path not within approved directory' }, 403);
    }

    if (!fs.existsSync(resolvedPath)) {
      return c.json({ error: 'File not found -- PDF may still be generating' }, 404);
    }

    const ext = path.extname(resolvedPath).toLowerCase();
    const ALLOWED_EXTENSIONS = new Set(['.pdf', '.md']);
    if (!ALLOWED_EXTENSIONS.has(ext)) {
      return c.json({ error: 'File type not served' }, 415);
    }
