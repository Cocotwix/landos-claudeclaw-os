...
import type { PlatformClass, SearchMethod, NavigationStrategy } from './website-intelligence.js';
...
interface Row {
  ...
  nav_patterns: string; auth_required: number; known_limitations_json: string; confidence: string;
...
export function rememberPlatform(urlOrHost: string, patch: PlatformIntelPatch, actor = 'browser-intelligence'): PlatformIntel {
  ...
  getLandosDb().prepare(
    `INSERT INTO landos_platform_intel (platform, classification, search_methods_json, validated_strategy_json, nav_patterns, auth_required, known_limitations_json, confidence, times_used, times_succeeded, last_validated_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(platform) DO UPDATE SET classification=excluded.classification, search_methods_json=excluded.search_methods_json,
       ...
       known_limitations_json=excluded.known_limitations_json, confidence=excluded.confidence, times_used=excluded.times_used,
       times_succeeded=excluded.times_succeeded, last_validated_at=excluded.last_validated_at, updated_at=excluded.updated_at`,
  ).run(next.platform, String(next.classification), JSON.stringify(next.searchMethods), next.validatedStrategy ? JSON.stringify(next.validatedStrategy) : null, next.navPatterns, next.authRequired ? 1 : 0, JSON.stringify(next.knownLimitations), next.confidence, next.timesUsed, next.timesSucceeded, next.lastValidatedAt, now);
